...
export function ReportsTab({ data }: { data: ReportsData }) {
  ...
  const expensesByCategory = data.expenses.reduce((acc, expense) => {
    const category = expense.category || t('accounting.uncategorized')
    if (!acc[category]) {
      acc[category] = 0
    }
  ...
  const handleExportCSV = () => {
    const csvData = [
      [t('common.type'), t('common.description'), t('common.amount'), t('common.date'), t('accounting.client') + '/' + t('common.category')],
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-lg border p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-semibold">{t('accounting.financialSummary')}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 rounded-lg bg-green-50 dark:bg-green-400/10 border border-green-200 dark:border-green-300/20">
            <p className="text-sm text-green-700 dark:text-green-300 font-medium mb-1">{t('accounting.annualRevenue')}</p>
          ...
          <div className="p-4 rounded-lg bg-red-50 dark:bg-red-400/10 border border-red-200 dark:border-red-300/20">
            <p className="text-sm text-red-700 dark:text-red-300 font-medium mb-1">{t('accounting.annualExpenses')}</p>
          ...
          <div className={`p-4 rounded-lg border ${profitYear >= 0 ? 'bg-blue-50 dark:bg-blue-400/10 border-blue-200 dark:border-blue-300/20' : 'bg-orange-50 dark:bg-orange-400/10 border-orange-200 dark:border-orange-300/20'}`}>
            ...
            <p className="text-xs mt-1 text-muted-foreground">
              {t('accounting.margin')}: {profitMargin.toFixed(1)}%
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Top 5 Clients */}
      <div className="bg-card rounded-lg border p-6">
        <h3 className="text-lg font-semibold mb-4">{t('accounting.top5ClientsByRevenue')}</h3>
        {topClients.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-4">{t('accounting.noClientWithRevenue')}</p>
        ) : (
      ...
      <div className="bg-card rounded-lg border p-6">
        <h3 className="text-lg font-semibold mb-4">{t('accounting.top5ExpenseCategories')}</h3>
        {topCategories.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-4">{t('accounting.noExpensesRecorded')}</p>
        ) : (
          <div className="space-y-3">
      ...
      <div className="bg-card rounded-lg border p-6">
        <h3 className="text-lg font-semibold mb-4">{t('accounting.annualTaxSummary')}</h3>
        <div className="space-y-3 text-sm">
          <div className="flex justify-between py-2 border-b">
            <span className="text-muted-foreground">{t('accounting.grossRevenue')}</span>
            <span className="font-semibold">{data.revenue.year.toFixed(2)} $</span>
          </div>
          <div className="flex justify-between py-2 border-b">
            <span className="text-muted-foreground">{t('accounting.deductibleExpenses')}</span>
            <span className="font-semibold">-{totalExpenses.toFixed(2)} $</span>
          </div>
          <div className="flex justify-between py-2 border-b">
            <span className="text-muted-foreground">{t('accounting.taxableNetIncome')}</span>
            <span className="font-bold text-lg">{profitYear.toFixed(2)} $</span>
          </div>
          <p className="text-xs text-muted-foreground mt-4">
            {t('accounting.taxDisclaimer')}
          </p>
        </div>
      </div>
    </div>
