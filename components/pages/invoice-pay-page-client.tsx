      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="max-w-2xl w-full">
          ...
          <div className="bg-card border rounded-xl shadow-lg p-8 text-center">
            <h1 className="text-2xl font-bold mb-2">{t('payment.unavailable')}</h1>
      ...
      <div className="max-w-3xl mx-auto py-8">
        <div className="text-center mb-8">
          ...
          {isPaid && invoice.paidAt && (
            <p className="text-sm text-green-600 dark:text-green-400 mt-2">
              {t('payment.paidOn')} {new Intl.DateTimeFormat(t('common.locale')).format(new Date(invoice.paidAt))}
            </p>
          )}
        </div>
        ...
        <div className="bg-card border rounded-xl shadow-lg p-6 mb-6">
          ...
          <div className="flex items-start justify-between mb-6 pb-6 border-b">
            <div>
              <p className="text-sm text-muted-foreground mb-1">{t('payment.from')}</p>
            ...
            <div className="text-right">
              <p className="text-sm text-muted-foreground mb-1">{t('payment.to')}</p>
          ...
          <div className="overflow-x-auto rounded-lg border mb-6">
            <table className="w-full text-sm">
                ...
                {invoice.items.map((item) => (
                  <tr key={item.id} className="border-t">
                    ...
                    <td className="py-3 px-4 text-muted-foreground">
                      {new Intl.DateTimeFormat(t('common.locale'), { timeZone: 'UTC' }).format(
                        new Date(item.date)
                      )}
                    </td>
              ...
              <tfoot>
                ...
                <tr className="border-t">
                  <td className="py-3 px-4" colSpan={2}>
                    <span className="text-base font-semibold">{t('payment.totalToPay')}</span>
                  </td>
