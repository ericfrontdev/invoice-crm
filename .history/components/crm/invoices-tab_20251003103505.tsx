...
export function InvoicesTab({
  ...
  const projectInvoices = client.invoices.filter((inv) => inv.project !== null && inv.status !== 'archived')
  const punctualInvoices = client.invoices.filter((inv) => inv.project === null && inv.status !== 'archived')
  const archivedInvoices = client.invoices.filter((inv) => inv.status === 'archived')
  ...
  const doBatchAction = async (action: 'delete' | 'archive') => {
    ...
    try {
      ...
      if (action === 'delete') {
        ...
        setToast({ type: 'success', message: `${selectedInvoiceIds.length} facture(s) supprimée(s)` })
        ...
        setToast({ type: 'success', message: `${selectedInvoiceIds.length} facture(s) archivée(s)` })
      }
  ...
  const doAction = async (invoiceId: string, kind: 'send' | 'paid' | 'delete' | 'unarchive') => {
    try {
      setBusyId(invoiceId)
      let url = ''
      let method = 'POST'
      ...
      const res = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: method === 'POST' ? JSON.stringify({ invoiceId }) : method === 'PATCH' ? JSON.stringify({ status: 'draft' }) : undefined,
      })
      if (!res.ok) {
  ...
  const renderInvoiceTable = (invoices: Invoice[], emptyMessage: string, isArchived = false) => {
    ...
    const invoiceIds = invoices.map((inv) => inv.id)
    const allSelected = invoiceIds.every((id) => selectedInvoiceIds.includes(id))
    const someSelected = invoiceIds.some((id) => selectedInvoiceIds.includes(id))
    const selectedFromThisTable = invoiceIds.filter((id) => selectedInvoiceIds.includes(id))
  ...
  return (
    <>
      ...
      <div className="space-y-3">
        ...
        {archivedInvoices.length > 0 && (
          <div className="flex justify-center py-4">
            ...
            >
              <Archive className="h-4 w-4 mr-2" />
              {isArchived ? 'Masquer les archives' : `Voir les factures archivées (${archivedInvoices.length})`}
            </Button>
          </div>
        )}
      </div>
      ...
      >
        <div className="w-full bg-slate-100 dark:bg-slate-800 py-8 px-4 md:px-6">
          <div className="container mx-auto">
            <h3 className="text-lg font-semibold mb-3">Factures archivées</h3>
            <div className="bg-card rounded-lg border overflow-hidden">
              {renderInvoiceTable(
                archivedInvoices,
