...
export function ClientsView({ clients, showArchived }: { clients: Client[]; showArchived: boolean }) {
  ...
  const handleConfirmArchive = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setToast({ type: 'error', message: "Erreur lors de l'archivage du client" })
        setTimeout(() => setToast(null), 3000)
        return
      }
  ...
  const handleRestore = async (client: Client) => {
    try {
      ...
      if (!res.ok) {
        setToast({ type: 'error', message: 'Erreur lors de la restauration du client' })
        setTimeout(() => setToast(null), 3000)
        return
      }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/clients/${clientToDelete.id}?permanent=true`, {
        method: 'DELETE',
      })
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button
            ...
            size="sm"
            onClick={() => {
              const newUrl = showArchived ? '/clients' : '/clients?archived=true'
              router.push(newUrl)
            }}
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="text-sm text-muted-foreground">
                    Archivé le {client.archivedAt ? new Date(client.archivedAt).toLocaleDateString('fr-FR') : '-'}
                  </div>
                </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Voulez-vous archiver le client &ldquo;{clientToArchive?.name}&rdquo; ?
                  Le client sera masqué de la liste principale mais ses factures et projets seront conservés.
                  Vous pourrez le restaurer à tout moment depuis la section &quot;Clients archivés&quot;.
                </AlertDialogDescription>
              </AlertDialogHeader>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Êtes-vous sûr de vouloir supprimer définitivement le client &ldquo;{clientToDelete?.name}&rdquo; ?
